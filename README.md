...
### baseproduct.py
Базовый абстрактный класс-шаблон для класса Product

#### Методы
- `__init__(*args)` — абстрактный метод-конструктор.
- `__str__()` — абстрактный метод для преобразования объекта в строку.
- `__add__(other)` — абстрактный метод для сложения объектов.
- `@classmethod new_product(cls, product)` — абстрактный метод для создания нового объекта.

#### Свойства
- `price` (геттер и сеттер) — абстрактное свойство для получения и установки цены объекта.

### mixinlog.py
`MixinLog` — миксин, предназначенный для вывода информации об объекте в момент его создания. Он переопределяет методы `__init__` и `__repr__`.

#### Методы
- `__init__(*args)`:
  - Сохраняет переданные аргументы объекта в атрибуте `__init_args`.
  - Выводит строковое представление объекта, используя `__repr__`.

- `__repr__()`:
  - Возвращает строковое представление объекта в формате `<ClassName(args)>`.

#### Особенности
- Может быть добавлен в любой класс для автоматического вывода информации о создании объекта.
- Форматирует вывод так, чтобы он был удобен для отладки.
- Создаёт приватный атрибут __init_args в который сохраняются все атрибуты, используемые при инициализации объекта 

#### Использование
Унаследуйте класс от `MixinLog`, чтобы получить функциональность вывода информации об объекте:

```python
from estore.mixinlog import MixinLog

class MyClass(MixinLog):
...
#### Особенности
При попытке добавления продукта с нулевым количеством, выбрасывается ошибка ValueError 
...
- `average_price()`: Возвращает среднюю стоимость всех продуктов в категории или 0, если их нет. 
